...
config_after_install() {
    echo -e "${yellow}For security reasons, you need to forcefully modify the port and account password${plain} after installation/update is complete"
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red} failed to detect x-ui version, which may exceed the Github API limit. Please try again later, or manually specify x-ui version to install ${plain}"
            exit 1
        fi
        echo -e "Detected the latest version of dx-ui: ${last_version}, started installation"
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://github.com/vaxilu/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
            echo -e "${red} Failed to download x-ui, please make sure your server can download Github's file ${plain}"
            exit 1
        fi
    else
        last_version=$1
    ...
    config_after_install
    #echo -e "If it is a new installation, the default web page port is ${green}54321${plain}, and the default username and password are ${green}admin${plain}"
    #echo -e "Please make sure that this port is not occupied by other programs, ${yellow} and make sure that port 54321 is open${plain}"
    # echo -e "If you want to change 54321 to another port, enter the x-ui command to modify it, and also make sure that the port you modify is also open"
    #echo -e ""
    ...
    echo -e ""
    echo -e "dx-ui management script usage: "
    echo -e "---------------------------------------------------------------------------------"
    ...
    echo -e "x-ui disable    - Cancel x-ui to start automatically at startup"
